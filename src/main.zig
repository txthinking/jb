...
pub fn main() !void {
    ...
    var i = std.crypto.random.int(u32);
    var tmp = try std.fmt.allocPrint(allocator, "/tmp/{d}.js0", .{i});
    defer allocator.free(tmp);
    tmp[tmp.len - 1] = 0;
    const jb = @import("./jb.zig");
    var ok = try jb.jb(allocator, tmp[0 .. tmp.len - 1]);
    if (!ok) {
        return;
    }
    std.os.argv[1] = tmp[0 .. tmp.len - 1 :0].ptr;
    var zero: usize = 0;
